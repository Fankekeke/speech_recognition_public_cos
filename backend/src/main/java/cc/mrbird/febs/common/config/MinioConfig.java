...
public class MinioConfig implements InitializingBean {
     ...
     * @param fileName       文件名称
     */
    public String getObjectSaveTmpLocal(String fileName) throws Exception{
        //获取项目临时存储路径
        String savePathRoot = DirUtil.getAudioTmpPath();
        System.out.println(savePathRoot);
        String saveLocalTmpPath = savePathRoot + "" + fileName;

        MinioClient minioClient = new MinioClient(host, accessKey, secretKey);
        InputStream stream = minioClient.getObject(bucket, fileName);
        FileOutputStream fos = new FileOutputStream(saveLocalTmpPath);
