...
public class CreateGroup {
    ...
    //提供给主函数调用的方法
    public static void doCreateGroup(String requestUrl, String APPID, String apiSecret, String apiKey, Map<String, Object> params) {
        CreateGroup createGroup = new CreateGroup(requestUrl, APPID, apiSecret, apiKey);
        try {
            String resp = createGroup.doRequest(params);
    ...
    /**
     *
     * groupId 创建分组的标识，支持字母数字下划线，长度最大为32
     * groupName 创建分组的名称，长度最小为0，最大为256
     * groupInfo 创建分组的描述信息，长度最小为0，最大为256
     * @return 参数字符串
     */
    private String buildParam(Map<String, Object> requestParams) {
        String param = "{" +
                "    \"header\": {" +
                "        \"app_id\": \"" + APPID + "\"," +
                "        \"status\": 3" +
                ...
                //这里填上所需要的groupInfo
                "            \"groupInfo\": \"" + requestParams.get("groupInfo") + "\"," +
                "            \"createGroupRes\": {" +
                "                \"encoding\": \"utf8\"," +
                "                \"compress\": \"raw\"," +
                "                \"format\": \"json\"" +
