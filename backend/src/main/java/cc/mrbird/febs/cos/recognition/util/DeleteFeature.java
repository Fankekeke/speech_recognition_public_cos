...
public class DeleteFeature {
    ...
    //提供给主函数调用的方法
    public static void doDeleteFeature(String requestUrl, String APPID, String apiSecret, String apiKey, Map<String,Object> requestParams) {
        DeleteFeature deleteFeature = new DeleteFeature(requestUrl, APPID, apiSecret, apiKey);
        try {
            String resp = deleteFeature.doRequest(requestParams);
    ...
    private String buildParam(Map<String, Object> requestParams) throws IOException {
        String param = "{" +
                "    \"header\": {" +
                "        \"app_id\": \"" + requestParams.get("appId") + "\"," +
                "        \"status\": 3" +
                "    }," +
                "    \"parameter\": {" +
                "        \"s782b4996\": {" +
