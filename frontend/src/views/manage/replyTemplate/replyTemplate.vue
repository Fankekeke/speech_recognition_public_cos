<template>
  <a-card :bordered="false" class="card-area">
          ...
          <div :class="advanced ? null: 'fold'">
            <a-col :md="6" :sm="24">
              <a-form-item
                ...
                <a-input v-model="queryParams.templateId"/>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="24">
              <a-form-item
    ...
    <div>
      <div class="operator">
        <a-button type="primary" ghost @click="add">创建模板</a-button>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="templateContent" slot-scope="text, record">
          <a-tooltip v-if="record.templateContent">
              <template slot="title">
                {{ record.templateContent }}
              </template>
              {{ record.templateContent.slice(0, 60) }} {{ record.templateContent.length > 60 ? '...' : '' }}
          </a-tooltip>
          <span v-else>- -</span>
        </template>
...
import replyTemplateAdd from './replyTemplateAdd.vue'
moment.locale('zh-cn')

export default {
  name: 'record',
  components: {replyTemplateAdd, RangeDate},
  data () {
    return {
      recordAdd: {
        visiable: false
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/LLMReplyTemplate/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/LLMReplyTemplate/page', {
        ...params
      }).then((r) => {
